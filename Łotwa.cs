...
{
    public partial class Łotwa : Form
    {
        private User user;
        public Łotwa(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoŁotwa1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInRygaAsync()
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime RygaTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoŁotwa.Text = $"Aktualna godzina w Ryga: {RygaTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInRygaAsync();
        }

        private void InfoŁotwa_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoŁotwa1_TextChanged(object sender, EventArgs e)
        {
