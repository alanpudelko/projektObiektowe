    ...
    public partial class Dania : Form
    {
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoDania1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInCopenhagenAsync()
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime CopenhagenTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoDania.Text = $"Aktualna godzina w Copenhagen: {CopenhagenTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInCopenhagenAsync();
        }

        private void InfoDania_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoDania1_TextChanged(object sender, EventArgs e)
        {
