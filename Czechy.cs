...
{
    public partial class Czechy : Form
    {
        private User user;

        public Czechy(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoCzechy1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInPragueAsync()
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime PragueTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoCzechy.Text = $"Aktualna godzina w Prague: {PragueTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInPragueAsync();
        }

        private void InfoCzechy_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoCzechy1_TextChanged(object sender, EventArgs e)
        {
