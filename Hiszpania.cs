...
{
    public partial class Hiszpania : Form
    {
        private User user;
        public Hiszpania(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoHiszpania1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInMadridAsync()
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime MadridTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoHiszpania.Text = $"Aktualna godzina w Madrid: {MadridTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInMadridAsync();
        }

        private void InfoHiszpania_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoHiszpania1_TextChanged(object sender, EventArgs e)
        {
