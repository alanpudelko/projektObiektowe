...
{
    public partial class Węgry : Form
    {
        private User user;
        public Węgry(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoWęgry1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInBudapestAsync()
        {
            try
            {
                string apiKey = "22feb2021db55d19eb0608f7237cb536";
                string apiUrl = $"http://api.openweathermap.org/data/2.5/weather?q=Budapest&appid={apiKey}&units=metric";
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime BudapestTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoWęgry.Text = $"Aktualna godzina w Budapest: {BudapestTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInBudapestAsync();
        }

        private void InfoWęgry_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoWęgry1_TextChanged(object sender, EventArgs e)
        {
