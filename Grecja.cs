...
{
    public partial class Grecja : Form
    {
        private User user;
        public Grecja(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoGrecja1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInAthensAsync()
        {
            try
            {
                string apiKey = "22feb2021db55d19eb0608f7237cb536";
                string apiUrl = $"http://api.openweathermap.org/data/2.5/weather?q=Athens&appid={apiKey}&units=metric";
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        DateTime AthensTime = DateTime.UtcNow.AddHours(timezoneOffset);

                        InfoGrecja.Text = $"Aktualna godzina w Athens: {AthensTime.ToString("HH:mm:ss")}";
                    }
                    else
                    {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInAthensAsync();
        }

        private void Grecja_Load(object sender, EventArgs e)
        {

        }

        private void InfoGrecja_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoGrecja1_TextChanged(object sender, EventArgs e)
        {
