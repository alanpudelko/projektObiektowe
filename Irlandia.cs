...
{
    public partial class Irlandia : Form
    {
        private User user;
        public Irlandia(User user)
        {
            InitializeComponent();
            this.user = user;
        ...
        private async Task DisplayWeatherAsync()
        {
            ...
            try
            {
                ...
                if (weatherData != null)
                {
                    InfoIrlandia1.Text = $"Aktualna temperatura w {city}, {countryCode}: {weatherData.Main?.Temp}°C";
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GetCurrentTimeInDublinAsync()
        {
            try
            {
        ...
        {
            base.OnLoad(e);
            await GetCurrentTimeInDublinAsync();
        }

        private void InfoIrlandia_TextChanged(object sender, EventArgs e)
        {

        }

        private void InfoIrlandia1_TextChanged(object sender, EventArgs e)
        {
